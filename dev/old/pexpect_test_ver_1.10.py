...
def def_wrong_creds():
    ...
    var_next_device = (0)
    print (space01+ 'ERROR: Bad credentials for IP [' + hostname_ip + ']' +space02)
    while True:
        print ('TESTING WRONG CREDS FUNCTION')
        var_next_device = (1)
        break
...
def def_ssh_new_key():
    ...
    if i == 2: # diffie_hellman_group1_sha1
        ...
        if i2 == 1: # SSH does not have the public key. Just accept it.
            print (space01 + 'Accepting public SSH key for IP address [' + hostname_ip + ']' + space02)
            child.sendline ('yes')
            child.expect ('[Pp]assword: ')
            child.sendline(password)
            child.expect('#')
        if i2 == 4: # expect 'password'
            child.sendline(password)
            child.expect('#')
    ...
    if i == 4: # Expecting '[Pp]assword: '
        ...
        child.sendline(password)
        i4 = child.expect([pexpect.TIMEOUT, ssh_newkey, diffie_hellman_group1_sha1, ssh_key_fail, '[Pp]assword: ', '#'])
        if i4 == 4:
            def_wrong_creds()
        if i4 == 5:
            def_expect_hash_tag()
