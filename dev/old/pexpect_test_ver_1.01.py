...
username = ('rdiaz')
commandTorun1 = ('interface fa1')
var_show_inventory = ('do show inventory')
var_show_ip_interface_brief = ('do show ip interface brief')
var_show_clock = ('do show clock')
password = ('Password01$')
...
s.expect('#')
print (space01 + 'Entering configuration terminal' + space02)

s.sendline('config t')
s.expect('#')
s.sendline(commandTorun1)
s.expect('#')
s.sendline(var_show_ip_interface_brief)
s.expect('#')
#s.expect('\n\n')
output =  s.before
...
s.sendline(var_show_clock)
s.expect('#')
output =  s.before
output = output.decode("utf-8")
print (output)

s.sendline(var_show_inventory + '\n')
s.expect('#')
