...
# Date: 20181022
# Python3
# Name: cisco_switch_pexpect_ver_1.00.py
...
def def_verify_password():
    ...
    if ssh_expect_list == 5: # expecting '#'
        print ('***[' + hostname_ip + '] Loging into IP***')
...
def def_send_show_command():
    ...
    with open (ui_site + '_' + ui_describe_file + '_' + var_re_match_hostname_g1 + '_' + hostname_ip + '.txt', 'a') as var_write_to_file:
            ...
            print ('***[' + hostname_ip + '] running [' + show_command + ']')
...
def def_ssh_to_host():
    ...
    if i == 0: # Timeout
        print('***[' + hostname_ip + '] ---ERROR--- Unable to ssh due to TIMEOUT***')
    ...
    if i == 1: # SSH does NOT have the public key. Just accept it.
        print ('***[' + hostname_ip + '] Accepting public SSH key***')
        child.sendline ('yes')
    ...
    if i == 2: # diffie_hellman_group1_sha1
        print ('***[' + hostname_ip + '] --WARNING-- No matching key exchange method found. Their offer: diffie-hellman-group1-sha1***')
    ...
    if i == 4: # Expecting '[Pp]assword: '
        print ('***['+hostname_ip+ '] Already have SSH keys')
        child.sendline(password)
        def_verify_password()
...
def def_main():
    ...
    if var_next_device == 1:
        print ('***[' + hostname_ip + '] Unable to login. Moving on to next device/ip in the list/file.***')
